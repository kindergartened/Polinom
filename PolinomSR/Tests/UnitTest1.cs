...
namespace Tests
{
    ...
    public class PolynomialTests
    {
        ...
        {
            Polinom p1 = new Polinom(new double[] { -1, 2, 5 }); // 5x^2 + 2x^1 - 1
            Polinom result = p1.Integral(); // 1,67x^3+x^2-x+1
            Console.WriteLine(result);
            CollectionAssert.AreEqual(new Polinom(new double[] { 1.6666666666666667, 1, -1 }).Coefs, result.Coefs);
        }
        [TestMethod]
        public void TestDerative()
        {
            Polinom p1 = new Polinom(new double[] { -1, 2, 5 }); // 5x^2 + 2x^1 - 1
            Polinom result = p1.DerivativePolinom(); // 10x+2
        ...
        {
            Polinom p1 = new Polinom(new double[] { 1,1 }); // 4x^3 - 2x^2 + 3x + 1
            Polinom p2 = new Polinom(new double[] { 1, 3, -2, 4 }); // x + 1
        ...
        [TestMethod]
        public void RazdeleniePolynomials_ReturnsCorrectResult2()
        {
            Polinom p1 = new Polinom(new double[] { 1, 1 }); 
            Polinom p2 = new Polinom(new double[] { 0 }); 
